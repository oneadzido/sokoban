...
- **90 levels** in the standard Sokoban `.sbl` format, sourced from the classic XSokoban collection
...
- **Solved overlay blur** — the board is blurred behind the solved label so the arrangement cannot be studied after the level is solved
...
The 90 levels shipped with the app are the classic **XSokoban** collection, distributed with the original XSokoban implementation for Unix by Joseph L. Traub.

The collection is split across two origins:

- **Levels 1–50** are the original Sokoban puzzles designed by **Thinking Rabbit Inc.**, Japan, in 1982 — the company that created the game.
- **Levels 51–90** are the **XSokoban Extra** set, contributed by the community and distributed alongside the original levels.

Both sets have been freely redistributed with Sokoban clones for over three decades. The levels ship here as-is, renamed only, with no content changes.

Each level is stored as its own `.sbl` file in `app/src/main/assets/levels/`, named `level.01.sbl` through `level.90.sbl`. The naming mirrors the original XSokoban screen files (`screen.1` through `screen.90`), with zero-padding so the files sort correctly.

The `.sbl` format uses the standard Sokoban character set:
...
Lines starting with `;` are comments and are ignored. Blank lines are ignored. Every level in the set is known solvable — the collection has been solved many thousands of times by humans and by Sokoban solvers written over the last three decades.
...
- **XSokoban** by Joseph L. Traub — the classic Unix Sokoban implementation and the 90-level collection that ships with this app
